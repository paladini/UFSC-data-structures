...
* A classe ListaCircular implementa a estrutura de dados lista encadeada, uma lista formada por
* uma "sentinel", que é um ponteiro para um objeto do tipo Elemento; e um "size", que indica o tamanho
...
class ListaCircular {
 ...
 public:
    ...
    /** Construtor padrão da ListaCircular.
    * O construtor padrão da ListaCircular constroi uma lista encadeada vazia e "sentinel" apontando para NULL.
    */
    ListaCircular() {
        size = 1;

        // Não sei se posso usar o seguinte:
        // Elemento<T> nodoSentinela = new Elemento<T>(NULL, this);
        // ou 
        // Elemento<T> nodoSentinela = new Elemento<T>(NULL, self);
        Elemento<T> nodoSentinela = new Elemento<T>(NULL, NULL);
        nodoSentinela->setProximo(nodoSentinela);
        sentinel = nodoSentinela;
    }

    /** Destrutor padrão da ListaCircular.
    * O destrutor padrão da ListaCircular destrói a lista encadeada atribuindo à "size" o valor 0 e deletando o ponteiro "sentinel".
    */
    ~ListaCircular() {
       destroiLista();
    }
