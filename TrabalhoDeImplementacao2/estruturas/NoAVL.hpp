...
class NoAVL : public NoBinario<T> {
 ...
 public:
    ...
    */
    ...
    std::vector<T> breadth_first(NoAVL<T>* raiz) const {
        std::vector<NoAVL<T>*> nodes;
        std::vector<T> items;
        if (raiz != nullptr) {
            nodes.push_back(raiz);
            
            for (NoAVL<T>* node : nodes) {
                if (node->getEsquerda() != nullptr) {
                    nodes.push_back(node->getEsquerda());
                }
                if (node->getDireita() != nullptr) {
                    nodes.push_back(node->getDireita());
                }
                items.push_back(node->getDado());
            }
        }
        return items;
    }
